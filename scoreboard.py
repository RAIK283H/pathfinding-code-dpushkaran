...
class Scoreboard:
    ...
    player_path_display = []
    player_nodes_hit_display = [] # Display for showing the count of nodes hit
    ...
    def __init__(self, batch, group):
        ...
        for index, player in enumerate(config_data.player_data):
            ...
            #Label for the nodes hit display
            self.player_path_display.append(
                (path_label, player))
            nodes_hit_count_label = pyglet.text.Label("Number of Nodes Traveled:",
                                                        x=0,
                                                        y=0,
                                                        font_name='Arial',
                                                        font_size=self.font_size, batch=batch, group=group, color=player[2][colors.TEXT_INDEX])
            self.player_nodes_hit_display.append((nodes_hit_count_label, player))
    ...
    def update_elements_locations(self):
        ...
        for index, (display_element, player) in enumerate(self.player_nodes_hit_display):
            display_element.x = config_data.window_width - self.stat_width
